...
const api = {
  ...
  async chartBalances(token, period) {
    const query = `
      query($period: ChartPeriod) {
        chart:chartBalances(period: $period)
      }
    `;
    const vars = { period }
  ...
  async createProject(token, { name, position }) {
    const query = `
    ...
    `;
    const vars = { name, position };
    const data = await client(token).request(query, vars);
    log(query, data);
    return data.action;
  },

  async updateProject(token, { id, name, position }) {
    const query = `
