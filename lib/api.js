...
const api = {
   ...
   * @param {string} [params.color] - цвет проекта.
   ...
   * @param {string|number} params.project_id - ID проекта.
   * @param {string} params.name - название элемента.
   * @returns {Promise<{project_item: Object, errors: string[]}>} объект с созданным элементом и списком ошибок.
   */
  async createProjectItem(token, { project_id, name }) {
    const query = `
      mutation($project_id:ID!, $name:String!) {
    ...
    `;
    const vars = { project_id: project_id.toString(), name };
    const data = await client(token).request(query, vars);
    log(query, data);
    return data.action;
  },
   ...
   * @returns {Promise<{project_item: Object, errors: string[]}>} объект с обновлённым элементом и списком ошибок.
   */
  async updateProjectItem(token, { id, name, position }) {
    const query = `
    ...
    `;
    const vars = { id: id.toString(), name, position };
    const data = await client(token).request(query, vars);
    log(query, data);
    return data.action;
  },
  ...
  async destroyProjectItem(token, id) {
    const query = `
      mutation($id:ID!) {
        action:destroyProjectItem(id: $id) {
          success
          errors
        }
      }
